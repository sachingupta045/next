            ...
            <div className="flex w-max animate-[mqrun_44s_linear_infinite]">
                ...
                <div className="flex shrink-0 items-center gap-[38px] pr-[38px] whitespace-nowrap">
                    {marqueeItems.map((item, index) => (
                        <div key={`set1-${index}`} className="inline-flex items-center gap-[10px] text-base text-[0.86rem] font-medium">
                            <i className="fas fa-circle text-base/55 text-[0.55rem]"></i>
                            {item}
                        </div>
                    ))}
                </div>
                ...
                <div className="flex shrink-0 items-center gap-[38px] pr-[38px] whitespace-nowrap" aria-hidden="true">
                    {marqueeItems.map((item, index) => (
                        <div key={`set2-${index}`} className="inline-flex items-center gap-[10px] text-base text-[0.86rem] font-medium">
                            <i className="fas fa-circle text-base/55 text-[0.55rem]"></i>
                            {item}
                        </div>
                    ))}
